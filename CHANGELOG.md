...
- `AGENTS.md` no longer ships with the package or the docs. It is generated by inlining the maintainer's `CLAUDE.md` and its `@`-imports, so it carried contributor workflow and references to unrelated internal projects — not consumer material. It was present in the tarball of 0.4.0–0.6.0 and published to hexdocs as `agents.html`. The file stays in the repo for cross-family reviewers.
- Dev-only deps bumped: sobelow 0.14.1→0.15.0, tidewave 0.6.0→0.9.0, styler 1.11.0→1.12.2, reach 2.7.5→2.8.2, ex_slop 0.4.2→0.4.4, ex_dna 1.5.3→1.5.4, bandit 1.12.0→1.12.5. No effect on consumers — ExUnitJSON ships with no runtime dependencies.
- README install snippet now shows `~> 0.6` instead of the stale `~> 0.4`.
